# Data preparation for ambulance placement optimisation

# Load required packages, installing any that are missing
required_packages <- c("stats19", "dplyr", "sf", "ggplot2", "viridis", "readr", "scales")
...
bbox_lonlat <- st_as_sfc(
  st_bbox(
    c(xmin = -2.20, xmax = -1.60, ymin = 52.30, ymax = 52.65),
    crs = 4326
  )
)
...
download_stats19_year <- function(year) {
  ...
  get_stats19(year = year, type = "collision", output_format = "sf") %>%
    mutate(year_downloaded = year)
}
...
if (filtered_collisions == 0) {
  stop("No collisions found inside the selected bounding box.")
}
...
if (nrow(collisions_joined) == 0) {
  stop("No collisions were assigned to grid cells.")
}
...
scenario_matrix <- matrix(
  ...
  ncol = length(scenario_years),
  dimnames = list(cell_ids, scenario_years)
)
...
scenario_weights_list <- lapply(
  seq_len(ncol(scenario_matrix)),
  function(k) scenario_matrix[, k]
)
...
candidate_pool <- demand_centroids_det %>%
  mutate(
    candidate_pool_id = row_number(),
    sample_prob = weight / sum(weight)
  )

# Sample candidate sites
if (nrow(candidate_pool) > max_candidate_sites) {
  candidate_sites <- candidate_pool %>%
    slice_sample(n = max_candidate_sites, weight_by = sample_prob)
} else {
  candidate_sites <- candidate_pool
}
...
# Save model input files required by Shared_helpers.R
